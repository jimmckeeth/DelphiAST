...
function TPanelColorFinder.TryGetWithContext(Node: TSyntaxNode; var Context: string): Boolean;
var
...
function TPanelColorFinder.GetFullNameWithContext(const BaseName: string; Node: TSyntaxNode): string;
begin
  Result := BaseName;
...
var
  ...
  FullName: string; 
begin
  Writeln(Format('Assignment found in %s at line %d:', [FCurrentFileName, Node.Line]));
